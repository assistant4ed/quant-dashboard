...
/* ============================================================
   IBKR Integration: Short-Term Trading
   ============================================================ */

var ibkrConnected = false;
var shorttermChart = null;
var realtimeChart = null;
var realtimeInterval = null;
var realtimeBarSize = '1min';
var realtimeTicker = '';
...
async function checkIbkrStatus() {
  ...
  try {
    var resp = await fetch('/api/ibkr/status');
    if (!resp.ok) throw new Error('HTTP ' + resp.status);
    var result = await resp.json();
    var gw = result.data && result.data.gateway;
    if (gw && gw.authenticated) {
      ibkrConnected = true;
      if (dot) dot.className = 'ibkr-status-dot ibkr-connected';
      if (text) text.textContent = 'IBKR Connected — Live Data Active';
      loadTop20Stocks();
      loadOptions30();
      ...
      if (text) text.textContent = 'IBKR Not Authenticated — Open https://localhost:5055 to login';
    }
  } catch (e) {
    if (dot) dot.className = 'ibkr-status-dot ibkr-disconnected';
    if (text) text.textContent = 'IBKR Gateway Offline';
  }
}
...
function startIbkrKeepalive() {
  setInterval(function() {
    if (ibkrConnected) {
      fetch('/api/ibkr/keepalive', { method: 'POST' }).catch(function() {});
    }
  }, 60000);
}

/* --- Top 20 Stocks Grid --- */
async function loadTop20Stocks() {
  var grid = document.getElementById('shortterm-grid');
  if (!grid) return;
  grid.innerHTML = '<p style="color:var(--text-muted);padding:20px">' + t('loading') + '</p>';

  try {
    var resp = await fetch('/api/ibkr/top20');
    if (!resp.ok) throw new Error('HTTP ' + resp.status);
    var result = await resp.json();
    ...
    grid.innerHTML = '';
    if (stocks.length === 0) {
      grid.innerHTML = '<p style="color:var(--text-muted);padding:20px">Waiting for market data... (data may need a second request)</p>';
      /* IBKR needs a second call for snapshots to populate */
      setTimeout(loadTop20Stocks, 3000);
      return;
    }

    stocks.forEach(function(s) {
      var card = document.createElement('div');
      card.className = 'st-stock-card';
      var changePct = parseFloat(s.change_pct) || 0;
      var changeClass = changePct >= 0 ? 'st-positive' : 'st-negative';
      var changeSign = changePct >= 0 ? '+' : '';
      var last = s.last || s.close || '--';
      var vol = s.volume ? formatVolume(parseInt(String(s.volume).replace(/[^0-9]/g, ''))) : '--';

      card.innerHTML =
        '<div class="st-card-header">' +
          '<span class="st-ticker">' + escapeHtml(s.symbol) + '</span>' +
          '<span class="st-price">$' + last + '</span>' +
        '</div>' +
        '<div class="st-card-body">' +
          '<span class="st-change ' + changeClass + '">' + changeSign + changePct.toFixed(2) + '%</span>' +
          '<span class="st-volume">Vol: ' + vol + '</span>' +
        '</div>';

      card.style.cursor = 'pointer';
      card.addEventListener('click', function() {
        document.getElementById('shortterm-ticker-select').value = s.symbol;
        runShorttermAnalysis(s.symbol);
      });
      grid.appendChild(card);
    });

    /* Populate dropdown */
    var select = document.getElementById('shortterm-ticker-select');
    if (select) {
      select.innerHTML = '<option value="">' + t('select.stock') + '</option>';
      stocks.forEach(function(s) {
        var opt = document.createElement('option');
        opt.value = s.symbol;
        opt.textContent = s.symbol + (s.last ? ' — $' + s.last : '');
        select.appendChild(opt);
      });
    }
  } catch (e) {
    grid.innerHTML = '<p style="color:var(--text-muted);padding:20px">Failed to load: ' + escapeHtml(String(e)) + '</p>';
  }
}

/* --- Short-Term Analysis --- */
async function runShorttermAnalysis(symbol) {
  var signals = document.getElementById('shortterm-signals');
  var indicators = document.getElementById('shortterm-indicators');
  if (signals) signals.innerHTML = '<p style="color:var(--text-muted)">' + t('loading') + '</p>';
  if (indicators) indicators.innerHTML = '';

  try {
    var resp = await fetch('/api/ibkr/analysis/' + encodeURIComponent(symbol));
    if (!resp.ok) throw new Error('HTTP ' + resp.status);
    var result = await resp.json();
    var data = result.data;

    /* Render intraday chart */
    renderShorttermChart(data);

    /* Render recommendation */
    if (signals) {
      var recClass = data.recommendation === 'BUY' ? 'rec-buy' : data.recommendation === 'SELL' ? 'rec-sell' : 'rec-hold';
      var html = '<div class="st-recommendation ' + recClass + '">';
      html += '<div class="st-rec-label">' + escapeHtml(symbol) + ' — ' + t('recommendation') + '</div>';
      html += '<div class="st-rec-value">' + data.recommendation + '</div>';
      html += '<div class="st-rec-reason">' + escapeHtml(data.recommendation_rationale || '') + '</div>';
      html += '</div>';

      html += '<div class="st-rationale-list">';
      (data.rationale || []).forEach(function(r) {
        html += '<div class="st-rationale-item">' + escapeHtml(r) + '</div>';
      });
      html += '</div>';
      signals.innerHTML = html;
    }

    /* Render indicator values */
    if (indicators && data.indicators) {
      var ind = data.indicators;
      var items = [
        { label: 'VWAP', value: '$' + (ind.vwap || 0).toFixed(2), color: 'var(--accent-blue)' },
        { label: 'RSI (14)', value: (ind.rsi || 0).toFixed(1), color: ind.rsi > 70 ? '#ef4444' : ind.rsi < 30 ? '#10b981' : 'var(--text-primary)' },
        { label: 'MACD', value: (ind.macd || 0).toFixed(4), color: ind.macd >= 0 ? '#10b981' : '#ef4444' },
        { label: 'Stoch K/D', value: (ind.stochastic_k || 0).toFixed(0) + '/' + (ind.stochastic_d || 0).toFixed(0), color: 'var(--text-primary)' },
        { label: t('support'), value: '$' + (ind.support || 0).toFixed(2), color: '#10b981' },
        { label: t('resistance'), value: '$' + (ind.resistance || 0).toFixed(2), color: '#ef4444' },
      ];
      indicators.innerHTML = '';
      items.forEach(function(item) {
        var div = document.createElement('div');
        div.className = 'st-indicator';
        div.innerHTML = '<div class="st-ind-label">' + escapeHtml(item.label) + '</div>' +
          '<div class="st-ind-value" style="color:' + item.color + '">' + item.value + '</div>';
        indicators.appendChild(div);
      });
    }
  } catch (e) {
    if (signals) signals.innerHTML = '<p style="color:var(--text-muted)">Analysis failed: ' + escapeHtml(String(e)) + '</p>';
  }
}

function renderShorttermChart(data) {
  var ctx = document.getElementById('shortterm-chart');
  if (!ctx) return;
  if (shorttermChart) shorttermChart.destroy();

  var candles = data.candles_5m || [];
  ...
  shorttermChart = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        x: { ticks: { maxTicksLimit: 15, font: { size: 9 } }, grid: { display: false } },
        y: { ticks: { callback: function(v) { return '$' + v.toFixed(0); } }, grid: { color: 'rgba(0,0,0,0.06)' } },
      },
    },
  });
...
/* --- Options Grid --- */
async function loadOptions30() {
  var grid = document.getElementById('options-grid');
  if (!grid) return;
  grid.innerHTML = '<p style="color:var(--text-muted);padding:20px">' + t('loading') + '</p>';

  try {
    var resp = await fetch('/api/ibkr/options30');
    if (!resp.ok) throw new Error('HTTP ' + resp.status);
    var result = await resp.json();
    var stocks = (result.data && result.data.options_underlyings) || [];

    if (stocks.length === 0) {
      setTimeout(loadOptions30, 3000);
      return;
    }
    ...
    stocks.forEach(function(s) {
      var card = document.createElement('div');
      card.className = 'opt-stock-card';
      var changePct = parseFloat(s.change_pct) || 0;
      var changeClass = changePct >= 0 ? 'st-positive' : 'st-negative';
      var last = s.last || s.close || '--';

      card.innerHTML =
        '<span class="opt-ticker">' + escapeHtml(s.symbol) + '</span>' +
        '<span class="opt-price">$' + last + '</span>' +
        '<span class="opt-change ' + changeClass + '">' + (changePct >= 0 ? '+' : '') + changePct.toFixed(2) + '%</span>';

      card.style.cursor = 'pointer';
      card.addEventListener('click', function() {
        document.getElementById('options-ticker-select').value = s.symbol;
        loadOptionsChain(s.symbol);
      });
      grid.appendChild(card);
    });

    /* Populate dropdown */
    var select = document.getElementById('options-ticker-select');
    if (select) {
      select.innerHTML = '<option value="">Select for options chain...</option>';
      (result.data.tickers || []).forEach(function(t) {
        var opt = document.createElement('option');
        opt.value = t;
        opt.textContent = t;
        select.appendChild(opt);
    ...
    if (grid) grid.innerHTML = '<p style="color:var(--text-muted);padding:20px">Options data loading...</p>';
  }
}

async function loadOptionsChain(symbol) {
  var container = document.getElementById('options-chain-data');
  if (!container) return;
  container.innerHTML = '<p style="color:var(--text-muted);padding:20px">' + t('loading') + '</p>';

  try {
    var resp = await fetch('/api/ibkr/options/' + encodeURIComponent(symbol));
    if (!resp.ok) throw new Error('HTTP ' + resp.status);
    var result = await resp.json();
    var data = result.data;

    var html = '<div class="options-info">';
    html += '<h4>' + escapeHtml(symbol) + ' Options Chain</h4>';

    if (data.options_info) {
      html += '<pre class="options-raw">' + escapeHtml(JSON.stringify(data.options_info, null, 2)) + '</pre>';
    }

    if (data.strikes) {
      html += '<div class="options-strikes">';
      html += '<h5>Available Strikes</h5>';
      html += '<pre class="options-raw">' + escapeHtml(JSON.stringify(data.strikes, null, 2)) + '</pre>';
      html += '</div>';
    }
    ...
    container.innerHTML = '<p style="color:var(--text-muted)">Failed to load options: ' + escapeHtml(String(e)) + '</p>';
  }
}

/* --- Real-Time Candle Chart --- */
function populateRealtimeDropdown() {
  var select = document.getElementById('realtime-ticker');
  if (!select) return;
  var allTickers = [
    'SPY', 'QQQ', 'AAPL', 'MSFT', 'NVDA', 'AMZN', 'META', 'GOOGL', 'TSLA', 'AMD',
    'JPM', 'V', 'NFLX', 'AVGO', 'COST', 'SPX', 'IWM',
  ];
...
function startRealtimeChart() {
  var ticker = document.getElementById('realtime-ticker').value;
  if (!ticker) return;
  realtimeTicker = ticker;

  var statusEl = document.getElementById('realtime-status');
  var startBtn = document.getElementById('realtime-start-btn');
  var stopBtn = document.getElementById('realtime-stop-btn');

  if (startBtn) startBtn.disabled = true;
  if (stopBtn) stopBtn.disabled = false;
  if (statusEl) { statusEl.textContent = 'Live — ' + ticker; statusEl.className = 'realtime-status realtime-live'; }

  /* Fetch initial data */
  fetchAndRenderCandles();

  /* Set up polling interval based on bar size */
  var intervalMs = 1000;
  if (realtimeBarSize === '5secs') intervalMs = 5000;
  else if (realtimeBarSize === '1min') intervalMs = 5000;
  else if (realtimeBarSize === '5min') intervalMs = 15000;
  else if (realtimeBarSize === '15min') intervalMs = 30000;
  else if (realtimeBarSize === '1h') intervalMs = 60000;

  realtimeInterval = setInterval(fetchAndRenderCandles, intervalMs);
}
...
function stopRealtimeChart() {
  ...
  if (statusEl) { statusEl.textContent = 'Stopped'; statusEl.className = 'realtime-status'; }
}
...
async function fetchAndRenderCandles() {
  ...
  try {
    var resp = await fetch('/api/ibkr/candles/' + encodeURIComponent(realtimeTicker) + '?bar=' + realtimeBarSize + '&period=1d');
    if (!resp.ok) return;
    var result = await resp.json();
    var candles = (result.data && result.data.candles) || [];

    if (candles.length === 0) return;

    renderCandlestickChart(candles);
    renderRealtimeInfoBar(candles);
  } catch (e) {
...
function renderCandlestickChart(candles) {
  var ctx = document.getElementById('realtime-candle-chart');
  if (!ctx) return;
  ...
  /* Use OHLC as a line chart with high/low range */
  var opens = candles.map(function(c) { return c.open; });
  var highs = candles.map(function(c) { return c.high; });
  var lows = candles.map(function(c) { return c.low; });
  var closes = candles.map(function(c) { return c.close; });
  var colors = candles.map(function(c) { return c.close >= c.open ? '#10b981' : '#ef4444'; });
  ...
  realtimeChart = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [
        {
          label: 'Range (H-L)',
          data: candles.map(function(c) { return [c.low, c.high]; }),
        ...
        tooltip: {
          callbacks: {
            ...
            label: function(ctx) {
              ...
              return ['O: $' + c.open.toFixed(2), 'H: $' + c.high.toFixed(2), 'L: $' + c.low.toFixed(2), 'C: $' + c.close.toFixed(2), 'V: ' + formatVolume(c.volume || 0)];
            },
          },
        },
      },
      scales: {
        x: { ticks: { maxTicksLimit: 20, font: { size: 8 } }, grid: { display: false } },
        y: {
          type: 'linear',
...
function renderRealtimeInfoBar(candles) {
  var bar = document.getElementById('realtime-info-bar');
  if (!bar || candles.length === 0) return;
  var last = candles[candles.length - 1];
  var change = last.close - last.open;
  var changePct = (change / last.open * 100);
  var changeClass = change >= 0 ? 'st-positive' : 'st-negative';

  bar.innerHTML =
    '<div class="rt-info-item"><span class="rt-label">O</span><span>$' + last.open.toFixed(2) + '</span></div>' +
    '<div class="rt-info-item"><span class="rt-label">H</span><span>$' + last.high.toFixed(2) + '</span></div>' +
    '<div class="rt-info-item"><span class="rt-label">L</span><span>$' + last.low.toFixed(2) + '</span></div>' +
    '<div class="rt-info-item"><span class="rt-label">C</span><span class="' + changeClass + '">$' + last.close.toFixed(2) + '</span></div>' +
    '<div class="rt-info-item"><span class="rt-label">Chg</span><span class="' + changeClass + '">' + (change >= 0 ? '+' : '') + changePct.toFixed(2) + '%</span></div>' +
    '<div class="rt-info-item"><span class="rt-label">Vol</span><span>' + formatVolume(last.volume || 0) + '</span></div>' +
    '<div class="rt-info-item"><span class="rt-label">Time</span><span>' + new Date(last.time).toLocaleTimeString() + '</span></div>';
}

/* --- Wire IBKR Events --- */
function wireIbkrEvents() {
  var analyzeBtn = document.getElementById('shortterm-analyze-btn');
  if (analyzeBtn) {
    analyzeBtn.addEventListener('click', function() {
      var ticker = document.getElementById('shortterm-ticker-select').value;
      if (ticker) runShorttermAnalysis(ticker);
    });
  }

  var optionsBtn = document.getElementById('options-load-btn');
  if (optionsBtn) {
    optionsBtn.addEventListener('click', function() {
      var ticker = document.getElementById('options-ticker-select').value;
      if (ticker) loadOptionsChain(ticker);
    });
  }

  var reconnectBtn = document.getElementById('ibkr-reconnect-btn');
  if (reconnectBtn) {
    reconnectBtn.addEventListener('click', checkIbkrStatus);
  }

  /* Realtime bar size buttons */
  var barBtns = document.querySelectorAll('#realtime-bar-group .toggle-btn');
  barBtns.forEach(function(btn) {
    btn.addEventListener('click', function() {
      barBtns.forEach(function(b) { b.classList.remove('active'); });
      btn.classList.add('active');
      realtimeBarSize = btn.dataset.bar;
      if (realtimeInterval) {
        stopRealtimeChart();
        startRealtimeChart();
      }
  ...
  var startBtn = document.getElementById('realtime-start-btn');
  if (startBtn) startBtn.addEventListener('click', startRealtimeChart);

  var stopBtn = document.getElementById('realtime-stop-btn');
  if (stopBtn) stopBtn.addEventListener('click', stopRealtimeChart);
}
