...
def create_app():
    ...
    def api_ibkr_status():
        """Check IBKR gateway connection and auth status."""
        try:
            ...
            return _error_response(
                "IBKR client not available", "MODULE_NOT_FOUND", 500,
            )
        except Exception as exc:
            logger.error("IBKR status check failed: %s", exc)
            return _error_response(str(exc), "IBKR_ERROR", 502)

    @app.route("/api/ibkr/keepalive", methods=["POST"])
    def api_ibkr_keepalive():
        """Keep IBKR session alive."""
        try:
            from ibkr_client import keep_session_alive
            result = keep_session_alive()
            return jsonify({
                "data": {"success": result is not None},
                "error": None,
                "meta": {"timestamp": datetime.now(timezone.utc).isoformat()},
            })
        except Exception as exc:
    ...
    @app.route("/api/ibkr/top20")
    def api_ibkr_top20():
        """Get real-time data for top 20 most-traded US stocks."""
        try:
            from ibkr_client import (
                get_market_snapshot_by_symbols,
                TOP_20_TICKERS,
            )
            snapshots = get_market_snapshot_by_symbols(TOP_20_TICKERS)
            return jsonify({
                "data": {
                    "stocks": snapshots,
                    "count": len(snapshots),
            ...
            logger.error("IBKR top20 failed: %s", exc)
            return _error_response(str(exc), "IBKR_ERROR", 502)

    @app.route("/api/ibkr/candles/<symbol>")
    def api_ibkr_candles(symbol):
        """Get intraday candles for a symbol.

        Query params:
            bar: Bar size (1min, 5min, 15min, 1h) default 1min
            period: Time period (1d, 2d, 5d) default 1d
        """
        if not TICKER_PATTERN.match(symbol):
            return _error_response(
                "Invalid symbol", "VALIDATION_FAILED", 400,
            )

        bar_size = request.args.get("bar", "1min")
        period = request.args.get("period", "1d")

        allowed_bars = {
            "1secs", "5secs", "10secs", "30secs",
            "1min", "2min", "5min", "10min", "15min", "30min", "1h",
        }
        allowed_periods = {"1d", "2d", "3d", "5d", "1w", "1m"}

        if bar_size not in allowed_bars:
            return _error_response(
                f"Invalid bar size: {bar_size}", "VALIDATION_FAILED", 400,
            )
        if period not in allowed_periods:
            return _error_response(
                f"Invalid period: {period}", "VALIDATION_FAILED", 400,
            )

        try:
            from ibkr_client import get_intraday_candles
            candles = get_intraday_candles(
                symbol.upper(), bar_size=bar_size, period=period,
            )
            return jsonify({
                "data": {
                    "symbol": symbol.upper(),
            ...
            logger.error("IBKR candles failed for %s: %s", symbol, exc)
            return _error_response(str(exc), "IBKR_ERROR", 502)

    @app.route("/api/ibkr/analysis/<symbol>")
    def api_ibkr_analysis(symbol):
        """Get short-term trading analysis with signals and rationale."""
        if not TICKER_PATTERN.match(symbol):
            return _error_response(
                "Invalid symbol", "VALIDATION_FAILED", 400,
            )
        ...
        try:
            from ibkr_client import get_short_term_analysis
            analysis = get_short_term_analysis(symbol.upper())
            if "error" in analysis:
                return _error_response(
                    analysis["error"], "ANALYSIS_FAILED", 404,
                )
            return jsonify({
                "data": analysis,
                "error": None,
            ...
            logger.error("IBKR analysis failed for %s: %s", symbol, exc)
            return _error_response(str(exc), "IBKR_ERROR", 502)

    @app.route("/api/ibkr/options/<symbol>")
    def api_ibkr_options(symbol):
        ...
        try:
            from ibkr_client import get_options_chain, get_option_strikes
            chain = get_options_chain(symbol.upper())
            if "error" in chain:
                return _error_response(
                    chain["error"], "OPTIONS_FAILED", 404,
                )

            # Also fetch strikes if we have a conid
            strikes = None
            if chain.get("conid"):
                strikes = get_option_strikes(chain["conid"])
            ...
            logger.error("IBKR options failed for %s: %s", symbol, exc)
            return _error_response(str(exc), "IBKR_ERROR", 502)

    @app.route("/api/ibkr/options30")
    def api_ibkr_options30():
        """Get options overview for the top 30 most-traded options."""
        try:
            from ibkr_client import TOP_30_OPTIONS, get_market_snapshot_by_symbols
            snapshots = get_market_snapshot_by_symbols(TOP_30_OPTIONS)
            return jsonify({
                "data": {
                    "options_underlyings": snapshots,
                    "count": len(snapshots),
                    "tickers": TOP_30_OPTIONS,
                },
                "error": None,
                "meta": {"timestamp": datetime.now(timezone.utc).isoformat()},
            })
        except Exception as exc:
            logger.error("IBKR options30 failed: %s", exc)
            return _error_response(str(exc), "IBKR_ERROR", 502)
    ...
    # ------------------------------------------------------------------
    # IBKR: Expanded endpoints
    # ------------------------------------------------------------------

    @app.route("/api/ibkr/reconnect", methods=["POST"])
    def api_ibkr_reconnect():
        """Attempt to reconnect to IBKR gateway with auto-retry."""
        try:
            from ibkr_client import auto_reconnect
    ...
    @app.route("/api/ibkr/top50")
    def api_ibkr_top50():
        """Get real-time data for top 50 most-traded US stocks."""
        try:
            from ibkr_client import get_top50_snapshots
            snapshots = get_top50_snapshots()
            return jsonify({
                "data": {
                    "stocks": snapshots,
                    "count": len(snapshots),
            ...
            logger.error("IBKR top50 failed: %s", exc)
            return _error_response(str(exc), "IBKR_ERROR", 502)
    ...
    @app.route("/api/ibkr/session-status")
    def api_ibkr_session_status():
        """Return full IBKR session manager health info."""
        try:
            ...
            return _error_response(
                "IBKR session module not available",
                "MODULE_NOT_FOUND",
                500,
            )
        except Exception as exc:
            logger.error("IBKR session status failed: %s", exc)
            return _error_response(str(exc), "IBKR_SESSION_ERROR", 500)

    @app.route("/api/ibkr/session/start", methods=["POST"])
    def api_ibkr_session_start():
        """Start the IBKR persistent session manager."""
        try:
            from ibkr_session import session_manager
            ...
            logger.error("IBKR session start failed: %s", exc)
            return _error_response(str(exc), "IBKR_SESSION_ERROR", 500)

    @app.route("/api/ibkr/session/stop", methods=["POST"])
    def api_ibkr_session_stop():
        """Stop the IBKR persistent session manager."""
        try:
            from ibkr_session import session_manager
            ...
            logger.error("IBKR session stop failed: %s", exc)
            return _error_response(str(exc), "IBKR_SESSION_ERROR", 500)
    ...
    def api_portfolio():
        """Get portfolio summary: account + positions."""
        try:
            from trading import get_portfolio_summary
            from ibkr_client import get_accounts
            accounts = get_accounts()
            if not accounts:
                return _error_response(
                    "No IBKR accounts found", "NO_ACCOUNTS", 404,
                )
            account_id = (
            ...
            logger.error("Trading module not available: %s", exc)
            return _error_response(
                "Trading module not available", "MODULE_NOT_FOUND", 500,
            )
        except Exception as exc:
            logger.error("Portfolio fetch failed: %s", exc)
            return _error_response(str(exc), "TRADING_ERROR", 502)

    @app.route("/api/trading/recommend/<ticker>", methods=["POST"])
    def api_trade_recommend(ticker):
        """Get AI trade recommendation for a ticker."""
        try:
            from trading import ai_trade_recommendation
            from data_sources import get_full_stock_profile, get_market_context
            from ai_analyst import analyze_stock
            ...
            stock_data = get_full_stock_profile(ticker_upper)
            market_context = get_market_context()
            analysis = analyze_stock(ticker_upper, stock_data, market_context)
            recommendation = ai_trade_recommendation(
                ticker_upper, analysis,
            )
            ...
            return jsonify({
                "data": {
                    ...
                    "input_data_summary": {
                        "fundamentals_fields": list(
                            stock_data.get("fundamentals", {})
                            .get("ratios", {})
                            .keys()
                        ),
                        "technicals_fields": list(
                            stock_data.get("technicals", {}).keys()
                        ),
                        "macro_indicators": list(
                            market_context.get("macro", {})
                            .get("indicators", {})
                            .keys()
                        ),
                        "data_sources": [
            ...
            logger.error("Trading/analysis module not available: %s", exc)
            return _error_response(
                "Required module not available", "MODULE_NOT_FOUND", 500,
            )
        except Exception as exc:
            logger.error("Trade recommendation failed for %s: %s", ticker, exc)
            return _error_response(str(exc), "RECOMMENDATION_FAILED", 502)

    @app.route("/api/trading/execute", methods=["POST"])
    def api_trade_execute():
        """Execute a trade (requires confirmation)."""
        try:
            from trading import execute_ai_trade
            from ibkr_client import get_accounts
            ...
            result = execute_ai_trade(
                account_id, recommendation, confirmed=confirmed,
            )
            ...
            return _error_response(str(exc), "TRADE_ERROR", 502)

    @app.route("/api/trading/orders")
    def api_live_orders():
        """Get live orders."""
        try:
            from trading import get_live_orders
            ...
            logger.error("Live orders fetch failed: %s", exc)
            return _error_response(str(exc), "TRADING_ERROR", 502)
    ...
    def api_cancel_order(order_id):
        ...
        try:
            ...
            result = cancel_order(account_id, order_id)
            return jsonify({
                "data": result,
                "error": None,
                "meta": {
    ...
    def api_options_unusual():
        ...
        try:
            ...
            auth_check = _get("/iserver/auth/status")
            if not auth_check or not auth_check.get("authenticated"):
                return _error_response(
                    "IBKR gateway not connected", "IBKR_DISCONNECTED", 503,
                )
            from options_scanner import scan_unusual_activity
            data = scan_unusual_activity()
            return jsonify({
                "data": data,
                "error": None,
                "meta": {
            ...
            logger.error("Unusual options scan failed: %s", exc)
            return _error_response(str(exc), "OPTIONS_ERROR", 502)
    ...
    # Singleton auto-trader instance (created on first use)
    _auto_trader_instance = {}

    def _get_auto_trader():
        """Get or create the singleton AutoTrader instance."""
        if "instance" not in _auto_trader_instance:
            from auto_trader import AutoTrader
            _auto_trader_instance["instance"] = AutoTrader()
        return _auto_trader_instance["instance"]

    @app.route("/api/auto-trade/start", methods=["POST"])
    def api_auto_trade_start():
        """Start auto-trading with optional config overrides."""
        try:
            from auto_trader import AutoTrader

            body = request.get_json() or {}
            config = body.get("config") or body
            # Remove non-config keys if the body was sent flat
            config = {
            ...
            if config:
                _auto_trader_instance["instance"] = AutoTrader(config=config)
            ...
            logger.error("Auto-trader module not available: %s", exc)
            return _error_response(
                "Auto-trader module not available",
                "MODULE_NOT_FOUND",
                500,
            )
        except Exception as exc:
            logger.error("Auto-trade start failed: %s", exc)
            return _error_response(str(exc), "AUTO_TRADE_ERROR", 502)
    ...
    def api_auto_trade_stop():
        ...
        try:
            trader = _get_auto_trader()
            body = request.get_json() or {}
            close_positions = body.get("close_positions", False)

            result = trader.stop(close_positions=close_positions)
                ...
                jsonify({
                    ...
                    "error": (
                        ...
                        if result.get("success")
                            ...
                            "code": "AUTO_TRADE_STOP_FAILED",
                            "message": result.get("error", "Stop failed"),
                        }
                    ),
                    "meta": {
                        "timestamp": datetime.now(timezone.utc).isoformat(),
            ...
            logger.error("Auto-trade stop failed: %s", exc)
            return _error_response(str(exc), "AUTO_TRADE_ERROR", 502)
    ...
    def api_auto_trade_status():
        ...
        try:
            ...
            logger.error("Auto-trade status failed: %s", exc)
            return _error_response(str(exc), "AUTO_TRADE_ERROR", 502)

    @app.route("/api/auto-trade/config", methods=["PATCH"])
    def api_auto_trade_config():
        ...
        try:
            ...
            result = trader.update_config(body)

            if "error" in result:
                return _error_response(
                    result["error"], "VALIDATION_FAILED", 400,
                )
            ...
            logger.error("Auto-trade config update failed: %s", exc)
            return _error_response(str(exc), "AUTO_TRADE_ERROR", 502)

    @app.route("/api/auto-trade/log")
    def api_auto_trade_log():
        """Get the auto-trading trade log."""
        try:
            trader = _get_auto_trader()
            log = trader.get_trade_log()
            return jsonify({
                "data": {"trades": log, "count": len(log)},
                "error": None,
                "meta": {
                    "timestamp": datetime.now(timezone.utc).isoformat(),
                },
            ...
            logger.error("Auto-trade log failed: %s", exc)
            return _error_response(str(exc), "AUTO_TRADE_ERROR", 502)

    # ------------------------------------------------------------------
    # Unified Broker Interface (IBKR with Alpaca fallback)
    # ------------------------------------------------------------------

    _broker_instance = None

    def _get_broker():
        """Lazy-initialise and return the singleton Broker instance."""
        nonlocal _broker_instance
    ...
    def api_broker_status():
        """Return which broker is active and connection health."""
        try:
            brk = _get_broker()
            return jsonify({
                "data": brk.get_status(),
            ...
            logger.error("Broker status check failed: %s", exc)
            return _error_response(str(exc), "BROKER_ERROR", 502)

    @app.route("/api/broker/account")
    def api_broker_account():
        """Get account info from the active broker."""
        try:
            brk = _get_broker()
            if not brk.is_connected():
                return _error_response(
                    "No broker connected. Configure IBKR gateway or "
                    "add Alpaca API keys to data/api_keys.json.",
                    "BROKER_DISCONNECTED",
                    503,
                )
            ...
            return jsonify({
                ...
                "error": None,
                "meta": {
                    "broker": brk.get_broker_name(),
                    "timestamp": datetime.now(timezone.utc).isoformat(),
                },
            })
        except Exception as exc:
            logger.error("Broker account fetch failed: %s", exc)
            return _error_response(str(exc), "BROKER_ERROR", 502)
    ...
    def api_broker_positions():
        ...
        try:
            ...
            logger.error("Broker positions fetch failed: %s", exc)
            return _error_response(str(exc), "BROKER_ERROR", 502)

    @app.route("/api/broker/order", methods=["POST"])
    def api_broker_place_order():
        ...
        try:
            ...
            if not symbol or not qty or not side:
                return _error_response(
                    "symbol, qty, and side are required fields",
                    "VALIDATION_ERROR",
                    400,
                )
            ...
            if body.get("limit_price") is not None:
                kwargs["limit_price"] = body["limit_price"]
            if body.get("stop_price") is not None:
                kwargs["stop_price"] = body["stop_price"]
            if body.get("time_in_force"):
                kwargs["time_in_force"] = body["time_in_force"]

            result = brk.place_order(
                symbol=symbol,
                qty=qty,
                side=side,
            ...
            logger.error("Broker order placement failed: %s", exc)
            return _error_response(str(exc), "ORDER_ERROR", 502)
    ...
    def api_broker_orders():
        ...
        try:
            ...
            orders = brk.get_orders(status=status)
            return jsonify({
                "data": {
                    "orders": orders,
            ...
            logger.error("Broker orders fetch failed: %s", exc)
            return _error_response(str(exc), "BROKER_ERROR", 502)

    @app.route("/api/broker/order/<order_id>", methods=["DELETE"])
    def api_broker_cancel_order(order_id):
        ...
        try:
            ...
            result = brk.cancel_order(order_id)
            return jsonify({
                "data": result,
                "error": None,
                "meta": {
            ...
            logger.error("Broker order cancel failed: %s", exc)
            return _error_response(str(exc), "CANCEL_ERROR", 502)
    ...
    # ------------------------------------------------------------------
    # Auto-start IBKR session manager
    # ------------------------------------------------------------------
    try:
        from ibkr_session import session_manager as _ibkr_session
        _ibkr_session.start()
        logger.info("IBKR session manager auto-started")
    except ImportError:
        logger.warning(
            "ibkr_session module not found; session manager not started"
        )
    except Exception as exc:
        logger.error("Failed to auto-start IBKR session manager: %s", exc)
